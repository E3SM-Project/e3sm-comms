# Before running:
# WordPress: Tools > Export > export pages (wordpress_pages.xml) and posts (wordpress_posts.xml)
# Copy those XMLs into ${IO_DIR}/input/e3sm_org_reviewer/
# e3sm.org > CMP Settings > CMP Advanced Setup: copy the list of pages to ${IO_DIR}/input/e3sm_org_reviewer/whitelisted_web_pages.txt

# Replace this with your IO dir!
IO_DIR=/global/homes/f/forsyth/ez/e3sm-comms-io

echo "Count of top-level Confluence pages:"
wc -l ${IO_DIR}/input/website_reviewer/confluence_top_levels_ALL.txt
echo "Count of whitelisted e3sm.org pages:"
wc -l ${IO_DIR}/input/e3sm_org_reviewer/whitelisted_web_pages.txt

echo "Step 1. Review Confluence"
echo "Note: this will require a Confluence login"
e3sm-comms-website-reviewer

echo "Step 2. Review e3sm.org"
cp ${IO_DIR}/output/website_reviewer/hierarchical_outline.txt ${IO_DIR}/input/e3sm_org_reviewer/hierarchical_outline.txt
cp ${IO_DIR}/output/website_reviewer/sensitive_terms.txt ${IO_DIR}/input/e3sm_org_reviewer/confluence_sensitive_terms.txt
e3sm-comms-e3sm-org-reviewer
echo "Output reports:"
echo "1. ${IO_DIR}/output/e3sm_org_reviewer/path_report.md"
echo "2. ${IO_DIR}/output/e3sm_org_reviewer/sensitive_terms.md"
echo "3. ${IO_DIR}/output/e3sm_org_reviewer/action_items.md"
